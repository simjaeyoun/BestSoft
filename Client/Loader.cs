...
namespace 로그인화면
{
    ...
    {
        public static List<Bitmap> spriteSheet = new List<Bitmap>();
        public static int spriteWidth; public static int spriteHeight;
        public static int x, y;

        public static bool[] Selected_Character = new bool[4] { true, true, true, true };

        public static void Check_Character(Ch_Color clr)
        {
            if (clr == Ch_Color.Black) { Selected_Character[(int)Ch_Color.Black] = false; }
            else if (clr == Ch_Color.Orange) { Selected_Character[(int)Ch_Color.Orange] = false; }
            else if (clr == Ch_Color.Green) { Selected_Character[(int)Ch_Color.Green] = false; }
            else if (clr == Ch_Color.Blue) { Selected_Character[(int)Ch_Color.Blue] = false; }
        }
        ...
        {
            if(Map == MapType.PS_First || Map == MapType.PS_Second)
            {
                Setup_Location(BaseForm_test.Me.clr);
            }
            else if(Map == MapType.PS_Final)
            {
                Final_Setup_Location((BaseForm_test.Me.clr));
            }
        }
        public static void Final_Setup_Location(Ch_Color clr)
        {
            switch (clr)
            {
                case Ch_Color.Black:
                    {
                        BaseForm_test.Me.location.X = 100;
                        BaseForm_test.Me.location.Y = 140;
                        BaseForm_test.SendLocation();
                        break;
                    }
                case Ch_Color.Orange:
                    {
                        BaseForm_test.Me.location.X = 350;
                        BaseForm_test.Me.location.Y = 140;
                        BaseForm_test.SendLocation();
                        break;
                    }
                ...
                case Ch_Color.Blue:
                    {
                        BaseForm_test.Me.location.X = 1050;
                        BaseForm_test.Me.location.Y = 140;
                        BaseForm_test.SendLocation();
                        break;
                    }
        ...
        public static void Setup_Location(Ch_Color clr)
        {
            switch (clr)
            {
                case Ch_Color.Black:
                    {
                        BaseForm_test.Me.location.X = 450;
                        BaseForm_test.Me.location.Y = 300 - 60;
                        BaseForm_test.SendLocation();
                        break;
                    }
                case Ch_Color.Orange:
                ...
                case Ch_Color.Green:
                    {
                        BaseForm_test.Me.location.X = 450 - 60;
