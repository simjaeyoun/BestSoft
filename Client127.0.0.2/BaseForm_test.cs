...
namespace 로그인화면
{
    public class BaseForm_test : Form
    {
        ...
        {
            StudentManager.StudentDic.TryGetValue("127.0.0.2", out My_StudentData);
        ...
        private void Check_Other()
        {
            ...
            if (StudentManager.StudentDic.TryGetValue("127.0.0.1", out StudentData) && lock1)
            {
                if (StudentData.clr != Ch_Color.UnKnown)
                {
                    ...
                    {
                        Other_player = new Player(StudentData.clr, "127.0.0.1");
                        lock1 = false;
                        this.Invalidate();
                    }
                    ...
                    {
                        Other_player = new Player(StudentData.clr, StudentData.Location.X, StudentData.Location.Y, "127.0.0.1");
                        lock1 = false;
                        this.Invalidate();
                    }
                    // label 추가
        ...
        {
            // 원본 이미지를 복사합니다.
            //Image bubbleimage = (Image)Properties.Resources.bubble
