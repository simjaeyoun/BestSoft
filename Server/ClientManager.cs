...
namespace 응소실프로젝트서버
{
    ...
    class ClientManager
    {
        ...
        //Next이동
        public event Action<Info_Next, string> NextAction = null;
        //Chat
        ...
        private async Task DataReceived(ClientData client)
        {
            ...
            try
            {
                ...
                {
                    Packet<object> ReceiveData = JsonConvert.DeserializeObject<Packet<object>>(JsonArray[i]);
                    int index = i;
                    Task.Run(() => ProcessData(client, ReceiveData, JsonArray[index]));
                    //인자는 receivedata, 
                }
                return;
        ...
        private async void ProcessData(ClientData client, Packet<object> ReceiveData, string receivedJson)
        {
            //클라이언트가 종료됐을때
            ...
            // 다음 맵 이동
            if (ReceiveData.packetType == PacketType.AboutNext)
            {
                if (NextAction != null)
                {
                    ...
                    if (Info != null)
                        client.StudentData.Info = Info;
                    //첫번째 null은 이 작업이 완료 되기 전까지는 endinvoke호출 불가
                    //일을 처리하는데 정보의 상태가 필요 없다면 null
                    Console.WriteLine("Next packet");
                    await Task.Run(() => NextAction.Invoke(client.StudentData.Info, ReceiveData.modifierID));
